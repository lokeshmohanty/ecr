...
page.on(
	"console",
	(m) => m.type() === "error" && notes.push(m.text().slice(0, 140)),
);
...
await page.addInitScript((base) => {
	try {
		localStorage.setItem(
			"ecr.connection",
			JSON.stringify({ baseUrl: base, token: "" }),
		);
...
await page.waitForSelector("[class*='row-grid'][class*='cursor-pointer']", {
	timeout: 20000,
});
...
check(
	"moving the cursor opens that thread",
	first.length > 0,
	first.slice(0, 44),
);
...
check(
	"moving again opens the next thread",
	second !== first,
	second.slice(0, 44),
);
...
check(
	"the sidebar offers an all-accounts group",
	sidebarText.toLowerCase().includes("all accounts"),
);
...
for (const account of health.accounts.map((a) => a.id)) {
	check(
		`the sidebar has a ${account} group`,
		sidebarText.toLowerCase().includes(account),
	);
}

check(
	"the footer names the account",
	(await footer(3)).length > 0,
	await footer(3),
);
...
check(
	"choosing an account scopes the query",
	scoped.includes("tag:main"),
	scoped,
);
check(
	"the footer follows the account",
	(await footer(3)).includes("@"),
	await footer(3),
);

const views = await page.textContent("nav");
check(
	"the account group lists every view",
	views.includes("Unread") && views.includes("Flagged"),
);
...
check(
	"suggestions appear while typing a query",
	suggestionText.includes("unread"),
	suggestionText.slice(0, 60),
);

await page.keyboard.press("Enter");
await page.waitForTimeout(2000);
check(
	"choosing a suggestion runs it",
	(await page.inputValue("header input")).includes("tag:unread"),
);
...
check(
	"the composer sits below the thread",
	layout.editorTop > layout.threadTop,
	`${Math.round(layout.threadTop)} vs ${Math.round(layout.editorTop)}`,
);
...
const startMode = await page.evaluate(() => {
	...
	return spans
		.find((s) => ["normal", "insert"].includes(s.textContent.trim()))
		?.textContent.trim();
});
...
check(
	"ctrl+p hides the composer",
	(await page.locator("textarea").count()) === 0,
);
check(
	"the draft is not lost",
	(await page.textContent("body")).includes("minimised"),
);
...
await page.waitForTimeout(300);
check(
	"ctrl+h leaves the composer without closing it",
	(await page.locator("textarea").count()) === 1,
);
check(
	"focus moved to the list",
	(await footer(2)).trim() === "list",
	await footer(2),
);
...
await page.waitForTimeout(1800);
check(
	"mail can still be browsed while the draft is open",
	(await page.locator("textarea").count()) === 1,
);
...
check(
	"recipient suggestions appear",
	addressList.includes("@"),
	addressList.slice(0, 60),
);
...
check(
	"the packages tab renders",
	settingsBody.includes("notmuch") && settingsBody.includes("self-managed"),
);
check(
	"every package is listed",
	["mbsync", "vdirsyncer", "imapnotify", "msmtp"].every((p) =>
		settingsBody.includes(p),
	),
);
...
check(
	"self-managed packages offer no config field",
	editable === 0,
	`${editable} fields`,
);
check(
...
check(
	"switching to ecr-managed reveals its config",
	afterEnable === 1,
	`${afterEnable} fields`,
);
