...
describe("query suggestions", () => {
  it("treats a bare word as a tag when one matches", () => {
    const values = suggestQuery("unre", ["unread", "inbox"]).map((s) => s.value);
  ...
  it("suggests every tag when the prefix has no fragment yet", () => {
    ...
    expect(suggestions.map((s) => s.value)).toEqual(["tag:inbox", "tag:unread"]);
  });
...
describe("ranking addresses", () => {
  it("matches on the address", () => {
    expect(rankAddresses(BOOK, "alice").map((a) => a.email)).toContain("alice@example.com");
  });

  it("matches on the display name", () => {
    expect(rankAddresses(BOOK, "zhang").map((a) => a.email)).toContain("azhang@other.com");
  });
  ...
  it("caps the list", () => {
    const big = Array.from({ length: 200 }, (_, i) => ({ name: null, email: `a${i}@x.com` }));
    expect(rankAddresses(big, "a").length).toBeLessThanOrEqual(8);
  });
});

describe("the recipient fragment being typed", () => {
  it("finds the fragment after the last comma on a header line", () => {
    expect(currentRecipientFragment("To: alice@x.com, bo", 19)).toEqual({
      header: "to",
      fragment: "bo",
      start: 17,
    });
  ...
  it("finds the first fragment on the line", () => {
    expect(currentRecipientFragment("To: ali", 7)).toEqual({
      header: "to",
      fragment: "ali",
      start: 4,
    });
  ...
  it("works on a Cc line", () => {
    expect(currentRecipientFragment("Cc: x@y.com", 11)?.header).toBe("cc");
  });

  it("returns null on a non-recipient line", () => {
    expect(currentRecipientFragment("Subject: hello", 14)).toBeNull();
  });

  it("returns null in the body", () => {
    expect(currentRecipientFragment("just some text", 5)).toBeNull();
  });

  it("returns null when the caret sits before the header colon", () => {
    expect(currentRecipientFragment("To: alice@x.com", 2)).toBeNull();
  });
});

describe("replacing the fragment with a chosen address", () => {
  it("substitutes the fragment and leaves a trailing comma", () => {
    const line = "To: alice@x.com, bo";
    const result = replaceRecipientFragment(line, 17, 19, "bob@corp.example.com");
    expect(result.text).toBe("To: alice@x.com, bob@corp.example.com, ");
    expect(result.caret).toBe(result.text.length);
  });

  it("substitutes a lone fragment", () => {
    const result = replaceRecipientFragment("To: ali", 4, 7, "alice@example.com");
    expect(result.text).toBe("To: alice@example.com, ");
  });
});
