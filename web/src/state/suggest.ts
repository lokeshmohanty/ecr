  ...
  if (withPrefix) {
    ...
    if (prefix === "tag" || prefix === "is") {
      for (const tag of tags) {
        if (tag.toLowerCase().startsWith(fragment)) push(`${prefix}:${tag}`, "tag");
...
export function parseAddress(raw: string): AddressEntry | null {
  ...
  return value.includes("@") && !/\s/.test(value) ? { name: null, email: value } : null;
}

export function rankAddresses(book: AddressEntry[], fragment: string): AddressEntry[] {
  ...
  scored.sort((a, b) => a.score - b.score || a.entry.email.localeCompare(b.entry.email));
...
export interface RecipientFragment {
  header: "to" | "cc" | "bcc";
  fragment: string;
  /** Index in the line where the fragment starts. */
  start: number;
}

/** What is being typed on a To/Cc/Bcc line, if the caret is on one. */
export function currentRecipientFragment(line: string, caret: number): RecipientFragment | null {
  const match = /^(to|cc|bcc):/i.exec(line);
  if (!match) return null;

  const headerEnd = match[0].length;
  if (caret < headerEnd) return null;

  const before = line.slice(0, caret);
  const lastComma = before.lastIndexOf(",");
  const from = Math.max(lastComma + 1, headerEnd);

  const raw = line.slice(from, caret);
  const leading = raw.length - raw.trimStart().length;

  return {
    header: match[1]!.toLowerCase() as RecipientFragment["header"],
    fragment: raw.trim(),
    start: from + leading,
  };
}

export function replaceRecipientFragment(
  line: string,
  start: number,
  end: number,
  address: string,
): { text: string; caret: number } {
  const text = `${line.slice(0, start)}${address}, ${line.slice(end)}`;
  return { text, caret: start + address.length + 2 };
}
