...
describe("message folding", () => {
  ...
  const toggle = (explicit: boolean | undefined, newest: boolean, expandNewest: boolean) =>
...
describe("mark queue to tag operations", () => {
  ...
  it("batches several messages into one operation list", () => {
    const ops = markToOps({ "a@x": staged(["archive"]), "b@x": staged(["flag"]) });
...
describe("staging arbitrary tags", () => {
  ...
  it("reads + and - explicitly", () => {
    expect(parseTagInput("+work -inbox")).toEqual({ add: ["work"], remove: ["inbox"] });
  });

  it("takes several tags at once", () => {
    expect(parseTagInput("a, b -c")).toEqual({ add: ["a", "b"], remove: ["c"] });
  });
