...
import { SECTION_IDS, isSectionId, type CustomView, type SectionId } from "../views";
import {
  PACKAGE_IDS,
  PACKAGE_LABELS,
  type Management,
...
function assign(
  ...
  if (key === "sidebarSections") {
    if (!Array.isArray(value)) {
      errors.push(`${at}: ${name} expects a list, such as ["mailboxes", "tags"]`);
      return;
    }
    const unknown = value.filter((v) => typeof v !== "string" || !isSectionId(v));
    if (unknown.length > 0) {
      errors.push(
  ...
  if (key === "sidebarCustom") {
    ...
    for (const entry of value) {
      if (typeof entry !== "object" || entry === null || Array.isArray(entry)) {
        errors.push(`${at}: every ${name} entry is a table with a name and a query`);
        return;
      }
      ...
      if (row.icon !== undefined && typeof row.icon !== "string") {
        errors.push(`${at}: the icon for ${JSON.stringify(row.name)} must be text`);
        return;
      }
      rows.push({
        name: row.name,
