...
import { defaultToml, withValue } from "../state/settings";
import { PACKAGE_IDS, PACKAGE_LABELS, type PackageId } from "../state/packages";
import { VimEditor } from "./VimEditor";

type Tab = "theme" | "device" | "packages" | "accounts" | "text";
			...
			<Show when={tab() === "packages"}>
				<div class="scroll-y flex-1 p-4">
					<p class="mb-4 max-w-2xl text-xs leading-relaxed text-ink-3">
						...
						owns the configuration — ecr reads it and never writes it.{" "}
						<b class="text-ink-2">Managed by ecr</b> hands the file to ecr,
						which generates it from the accounts on the{" "}
						<button
							type="button"
                  ...
                  */}
									...
									{/*
                    A self-managed package has nothing to edit, and a managed
                    one is generated from the accounts on the Accounts tab —
                    so neither shows a box. Five dead boxes filled the page and
                    implied the config mattered when it is ignored.
                  */}
									<Show
										when={managed()}
										fallback={
