    ...
    <>
      ...
      <Show when={tab() === "packages"}>
        <div class="scroll-y flex-1 p-4">
          <p class="mb-4 max-w-2xl text-xs leading-relaxed text-text-dim">
            <b class="text-text-secondary">Self-managed</b> means your system already
            owns the configuration — ecr reads it and never writes it, and anything
            entered here is ignored.{" "}
            <b class="text-text-secondary">Managed by ecr</b> hands the file to ecr,
          ...
          <For each={PACKAGE_IDS}>
            {(id) => {
              ...
              return (
                <section class="mb-3 rounded border border-border bg-bg-raised">
                  <div class="flex flex-wrap items-center gap-3 border-b border-border-soft px-3 py-2">
                    <div class="min-w-0 flex-1">
                      <div class="text-text-strong">{label.title}</div>
                      <div class="text-xs text-text-dim">
      ...
      <Show when={tab() === "text"}>
        ...
        <div class="flex shrink-0 items-center gap-2 border-b border-border px-4 py-2 text-xs text-text-dim">
          <span class="flex-1">preferences and keybindings</span>
          <button
            type="button"
            class="rounded border border-border px-2 py-1 hover:bg-bg-hover"
            onClick={() => {
    ...
    <For each={tabs}>
      {(tab) => (
        <button
          ...
          classList={{
            "bg-accent text-bg-raised": props.current === tab.id,
            "text-text-secondary hover:bg-bg-hover": props.current !== tab.id,
          }}
...
function Choice(props: { label: string; active: boolean; onSelect: () => void }) {
  return (
    <button
      ...
      classList={{
        "bg-accent text-bg-raised": props.active,
