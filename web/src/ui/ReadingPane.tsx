...
function MessageView(props: {
	...
	/**
	 * Why this message has no body, if it has none.
	 *
	 * Swallowing the failure left the pane saying *loading…* for as long as it
	 * was open, which is the one thing that was not happening — a deleted file, a
	 * message notmuch has indexed and can no longer read, or a server that went
	 * away mid-read all presented as a spinner that would never stop. It is held
	 * beside the resource rather than in it because a failure the resource keeps
	 * is re-thrown at whoever reads it, and this one is read while the pane
	 * renders.
	 */
			...
			<Show when={open()}>
				<div class="px-4 pb-4">
					<dl class="mb-3 grid grid-cols-[3rem_minmax(0,1fr)] gap-x-2 text-xs text-ink-3">
						<Recipients label="from" list={props.message.from} />
							...
							>
								{(reason) => (
									<div class="text-xs break-words text-blocking" role="alert">
										this message could not be read: {reason()}
									</div>
								)}
							</Show>
						}
					>
						{(loaded) => (
							<>
								<Show
									when={
										loaded().remote_resources_blocked > 0 &&
										!props.store.allowRemote()
									}
								>
									<div class="mb-2 flex items-center gap-2 rounded border border-rule bg-card px-2 py-1 text-xs">
										<span class="text-ink-3">
											...
											{loaded().remote_resources_blocked === 1 ? "" : "s"}{" "}
											blocked
										</span>
										<button
											type="button"
