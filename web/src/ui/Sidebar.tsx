  ...
  return (
    ...
    >
      <div ref={scroller} class="scroll-y flex-1 px-2 py-2">
        ...
        <Show when={!props.store.listsSearchable() && preferences().sidebarSections.includes("lists")}>
          <p class="mt-2 px-2 text-[11px] leading-snug text-ink-3">
            Mailing lists need <span class="mono">index.header.List</span> in your
            notmuch config, then <span class="mono">notmuch reindex '*'</span>.
            Run <span class="mono">ecr doctor</span>.
          </p>
        </Show>
      </div>
...
function Row(props: {
  ...
  const row = () => props.row;
  const active = () => row().kind === "view" && props.store.query() === row().query;
  const expandedGroup = () => props.store.expandedGroup() === row().group;
  const expandedSection = () =>
    !!row().section && props.store.expandedSections().has(`${row().group}:${row().section}`);
  const open = () => (row().kind === "group" ? expandedGroup() : expandedSection());
  const count = () => (row().counted ? props.store.countOf(row().query) : undefined);

  const label = () =>
    row().kind === "group" && row().name === ALL_ACCOUNTS ? "All accounts" : row().name;
  ...
  return (
    <button
      ...
      classList={{
        "mt-2 first:mt-0 uppercase tracking-widest": row().kind === "group",
        "mt-1 tracking-wide": row().kind === "section",
        "text-xs uppercase tracking-wide": row().kind === "view",
        "pl-1": row().indent === 0,
        "pl-3": row().indent === 1,
        "pl-6": row().indent === 2,
