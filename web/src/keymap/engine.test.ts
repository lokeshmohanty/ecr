...
describe("pane scoping", () => {
	it("Enter opens a thread in the list and selects a view in the sidebar", () => {
		const map = new Keymap();
		expect(press(map, "Enter", { pane: "list" })).toMatchObject({
			action: { kind: "open" },
		});
		expect(press(map, "Enter", { pane: "sidebar" })).toMatchObject({
			action: { kind: "select" },
		});
	});
	...
	it("a list binding does not fire in another pane", () => {
		const map = new Keymap();
		expect(press(map, "a", { pane: "list" })).toMatchObject({
			action: { kind: "archive" },
		});
		expect(press(map, "a", { pane: "detail" })).toEqual({
			type: "ignored",
			consumed: false,
		});
	});

	it("f flags in the list and toggles fullscreen in the detail pane", () => {
		const map = new Keymap();
		expect(press(map, "f", { pane: "list" })).toMatchObject({
			action: { kind: "toggleFlag" },
		});
		expect(press(map, "f", { pane: "detail" })).toMatchObject({
	...
	it("a global binding fires in every pane", () => {
		...
		for (const pane of ["sidebar", "list", "detail"] as Pane[]) {
			expect(press(map, "c", { pane }), pane).toMatchObject({
				action: { kind: "compose" },
			});
		}
	});
	...
	it("h and l move focus from any pane", () => {
		const map = new Keymap();
		for (const pane of ["sidebar", "list", "detail"] as Pane[]) {
			expect(press(map, "h", { pane })).toMatchObject({
				action: { kind: "focusLeft" },
			});
			expect(press(map, "l", { pane })).toMatchObject({
	...
	it("a pane binding wins over a global one on the same keys", () => {
		...
		expect(press(map, "z", { pane: "list" })).toMatchObject({
			action: { kind: "sync" },
		});
		expect(press(map, "z", { pane: "detail" })).toMatchObject({
			action: { kind: "help" },
		});
	});
	...
	it("describe narrows to a pane", () => {
		...
		expect(
			detail.some((b) => b.keys === "a" && b.panes?.includes("list")),
		).toBe(false);
	});
});
...
describe("multi-key sequences", () => {
	...
	it("abandons a sequence that cannot complete", () => {
		...
		expect(press(map, "q", { pane: "detail" })).toMatchObject({
	...
	it("z sequences only exist in the detail pane", () => {
		...
		map.reset();
		expect(press(map, "z", { pane: "list" })).toEqual({
			type: "ignored",
			consumed: false,
		});
	});
...
describe("modes and focus", () => {
	it("does not act on keys while a text field has focus", () => {
		const map = new Keymap();
		expect(press(map, "j", { editing: true })).toEqual({
			type: "ignored",
			consumed: false,
		});
	});
	...
	it("does not act on keys in insert mode", () => {
		const map = new Keymap();
		expect(press(map, "j", { mode: "insert" })).toEqual({
			type: "ignored",
			consumed: false,
		});
	});
	...
	it("escape leaves insert mode even though insert ignores other keys", () => {
		const map = new Keymap();
		expect(press(map, "Escape", { mode: "insert" })).toEqual({
			type: "cancelled",
			consumed: true,
		});
	});
...
describe("modifiers", () => {
	it("claims only the ctrl chords bound in the focused pane", () => {
		...
		expect(
			map.handle({ key: "h", ctrl: true }, "normal", false, "list"),
		).toMatchObject({
			type: "action",
		});
		expect(
			map.handle({ key: "t", ctrl: true }, "normal", false, "list"),
		).toEqual({
			type: "ignored",
			consumed: false,
		});
	...
	it("never claims a key held with meta, so browser shortcuts keep working", () => {
		const map = new Keymap();
		expect(
			map.handle({ key: "r", meta: true }, "normal", false, "list"),
		).toEqual({
			type: "ignored",
			consumed: false,
		});
...
describe("ctrl chords", () => {
	it("move focus from any pane", () => {
		const map = new Keymap();
		expect(
			map.handle({ key: "h", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "focusLeft" },
		});
		expect(
			map.handle({ key: "l", ctrl: true }, "normal", false, "sidebar"),
		).toMatchObject({
			action: { kind: "focusRight" },
		});
	});
	...
	it("work while a text field has focus, so you can leave an open editor", () => {
		const map = new Keymap();
		expect(
			map.handle({ key: "h", ctrl: true }, "insert", true, "detail"),
		).toMatchObject({
			action: { kind: "focusLeft" },
		});
	});
	...
	it("ctrl+b toggles the pinned split, even mid-edit", () => {
		const map = new Keymap();
		expect(
			map.handle({ key: "b", ctrl: true }, "insert", true, "detail"),
		).toMatchObject({
...
describe("scrolling and conversation movement in the detail pane", () => {
	...
	it("j and k still move the cursor in the list and the sidebar", () => {
		expect(press(map(), "j", { pane: "list" })).toMatchObject({
			action: { kind: "next" },
		});
		expect(press(map(), "j", { pane: "sidebar" })).toMatchObject({
			action: { kind: "next" },
		});
	});
	...
	it("ctrl-e and ctrl-y scroll a line at a time", () => {
		const m = map();
		expect(
			m.handle({ key: "e", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "scrollDown" },
		});
		expect(
			m.handle({ key: "y", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "scrollUp" },
		});
	});
	...
	it("ctrl-d and ctrl-u scroll half a screen", () => {
		const m = map();
		expect(
			m.handle({ key: "d", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "scrollDown", half: true },
		});
		expect(
			m.handle({ key: "u", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
	...
	it("ctrl-j and ctrl-k walk the conversation", () => {
		const m = map();
		expect(
			m.handle({ key: "j", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "nextMessage" },
		});
		expect(
			m.handle({ key: "k", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
	...
	it("ctrl-n and ctrl-p are the same movement", () => {
		const m = map();
		expect(
			m.handle({ key: "n", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "nextMessage" },
		});
		expect(
			m.handle({ key: "p", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "prevMessage" },
		});
	});
	...
	it("ctrl-h and ctrl-l still move between panes from the detail pane", () => {
		const m = map();
		expect(
			m.handle({ key: "h", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "focusLeft" },
		});
		expect(
			m.handle({ key: "l", ctrl: true }, "normal", false, "detail"),
		).toMatchObject({
			action: { kind: "focusRight" },
		});
	});
	...
	it("ctrl-b toggles the pinned split from anywhere", () => {
		for (const pane of ["sidebar", "list", "detail"] as Pane[]) {
			expect(
				map().handle({ key: "b", ctrl: true }, "normal", false, pane),
				pane,
			).toMatchObject({
				action: { kind: "togglePinned" },
			});
