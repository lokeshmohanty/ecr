...
check(
	"Space selects the row under the cursor",
	marked[0]?.includes("tape-selected"),
	marked[0],
);
...
check(
	"Space picks every row in the range",
	pickedCount === 3,
	`${pickedCount} picked`,
);
...
check(
	"Space leaves visual mode: a second Space toggles only the cursor",
	cursorToggled === 2,
	`${cursorToggled} picked`,
);

await press("Escape");
marked = await tapes();
const afterEscape = marked.filter((t) => t.includes("tape-selected")).length;
check(
	"Escape with no range clears the picks",
	afterEscape === 0,
	`${afterEscape} picked`,
);
...
check(
	"d stages a delete on every selected row",
	staged[0] === "D" && staged[1] === "D",
	staged.join("|"),
);

rows = await rowClasses();
const stagedHighlighted = rows.filter((c) => {
	const set = new Set(c.split(/\s+/));
	return set.has("bg-obligation-bg") || set.has("bg-neutral-bg");
}).length;
check(
	"d exits visual mode (range cleared, only the cursor)",
	stagedHighlighted === 1,
	`${stagedHighlighted} highlighted`,
);
const stagedStatus = await status();
check(
	"the status reports staged, not visual",
	/staged/.test(stagedStatus),
	stagedStatus,
);
...
staged = await badges();
check(
	"the tag is staged on both rows",
	staged[0] === "T" && staged[1] === "T",
	staged.join("|"),
);

const tagRows = await rowClasses();
const tagHighlighted = tagRows.filter((c) => {
	const set = new Set(c.split(/\s+/));
	return set.has("bg-obligation-bg") || set.has("bg-neutral-bg");
}).length;
check(
	"staging a tag exits visual mode (range cleared, only the cursor)",
	tagHighlighted === 1,
	`${tagHighlighted} highlighted`,
);
...
staged = await badges();
check(
	"x clears the queue once written",
	staged.every((b) => b === ""),
	staged.join("|"),
);
...
check(
	...
	/applied \d+/.exec(applied)?.[0] ?? applied.slice(0, 40),
);
...
const holiday = await page.evaluate(async () => {
	const response = await fetch(
		`/api/v1/threads?q=${encodeURIComponent("tag:holiday")}`,
	);
