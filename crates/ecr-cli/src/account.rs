...
pub fn add(options: AddOptions) -> anyhow::Result<()> {
    ...
    // Expunge on the far side, so archiving and deleting actually reach the
    // server rather than stopping at a local tag.
    //
    // `Expunge` is the knob, and it is easy to reach for the wrong one:
    // `Remove` propagates *mailbox* deletions, not message ones, and message
    // disappearance is already propagated because ecr emits no `Sync` line and
    // mbsync's default `Full` includes `Gone`. What the default `Expunge None`
    // meant was that the far copy was marked deleted and then never acted on —
    // so a message deleted in ecr sat in Gmail's inbox for ever, looking
    // deleted here and present there.
    //
    // Set for accounts added from now on and never written into one that
    // already exists: somebody's existing setup must not start removing mail
    // from a server because they upgraded. `ecr doctor` names the state either
    // way, and `expunge = "none"` is one line back.
...
pub fn vacation_show() -> anyhow::Result<()> {
    ...
    if let Some(from) = vacation.from {
        println!("  from     {}", as_date(from));
    }
    if let Some(until) = vacation.until {
        println!("  until    {}", as_date(until));
    }
    ...
    if let Ok(path) = vacation_ledger(&env) {
...
pub fn vacation_on(
    ...
    let vacation = ecr_core::vacation::Vacation {
        ...
        from: from.map(|d| parse_date(&d)).transpose()?,
        until: until.map(|d| parse_date(&d)).transpose()?,
        interval_days: every,
...
pub fn vacation_off() -> anyhow::Result<()> {
    let env = Env::from_process();
    let mut accounts = Accounts::load(&env)?;
...
pub fn vacation_forget() -> anyhow::Result<()> {
    let env = Env::from_process();
    let path = vacation_ledger(&env)?;
...
mod vacation_tests {
    ...
    /// the month, and for this feature that means being away for the wrong
    /// fortnight.
    #[test]
    fn dates_round_trip_through_the_epoch() {
        for date in ["2026-01-01", "2026-08-08", "1970-01-01", "2000-02-29"] {
            ...
            assert_eq!(as_date(seconds), date, "{date} did not survive");
        }
    }
    ...
    fn the_epoch_is_where_it_should_be() {
        ...
        assert_eq!(parse_date("2026-08-08").unwrap(), 1_786_147_200);
    }

    #[test]
    fn anything_that_is_not_a_date_is_refused_rather_than_guessed() {
        for text in ["08/08/2026", "tomorrow", "2026-13-01", "2026-08", ""] {
            assert!(parse_date(text).is_err(), "{text:?} was accepted");
        }
    }
}
...
pub async fn file(dry_run: bool) -> anyhow::Result<()> {
    let env = Env::from_process();
    let accounts = Accounts::load(&env)?;
