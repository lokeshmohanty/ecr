...
async fn dispatch() -> anyhow::Result<()> {
    ...
    match command {
        ...
        Command::Account { command } => match command {
            ...
            AccountCommand::File { dry_run } => account::filing::run(dry_run).await,
            AccountCommand::Vacation { command } => match command {
                VacationCommand::Show => account::vacation::show(),
                VacationCommand::On {
                    body,
                    subject,
                    until,
                    from,
                    every,
                } => account::vacation::on(&body, subject, from, until, every),
                VacationCommand::Off => account::vacation::off(),
                VacationCommand::Forget => account::vacation::forget(),
            },
